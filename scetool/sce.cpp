...
//	_hexdump(fp, " IV ", 0, mi->iv, METADATA_INFO_IV_LEN, FALSE);
	...
	fprintf(fp, "[*] Original Metadata Info:\n");
...
void _print_sce_file_keys(FILE *fp, sce_buffer_ctxt_t *ctxt)
{
	...
	//Get start of keys.
	auto keys = reinterpret_cast<u8 *>(ctxt->metash) + sizeof(metadata_section_header_t) * ctxt->metah->section_count;
...
static sce_buffer_ctxt_t *_sce_create_ctxt()
{
	...
	if((res = static_cast<sce_buffer_ctxt_t *>(malloc(sizeof(sce_buffer_ctxt_t)))) == nullptr)
		return nullptr;
	...
	//Allocate SCE header.
	res->sceh = static_cast<sce_header_t *>(malloc(sizeof(sce_header_t)));
	memset(res->sceh, 0, sizeof(sce_header_t));

	//Allocate metadata info (with random key/iv).
	res->metai = static_cast<metadata_info_t *>(malloc(sizeof(metadata_info_t)));
	...
	res->metah = static_cast<metadata_header_t *>(malloc(sizeof(metadata_header_t)));
	//memset(res->metah, 0, sizeof(metadata_header_t));
	//Allocate signature.
	res->sig = static_cast<signature_t *>(malloc(sizeof(signature_t)));
...
sce_buffer_ctxt_t *sce_create_ctxt_from_buffer(u8 *scebuffer)
{
	...
	res->sceh = reinterpret_cast<sce_header_t *>(scebuffer);
	...
	switch(res->sceh->header_type)
	{
		...
		{
			//SELF header.
			res->self.selfh = reinterpret_cast<self_header_t *>(res->scebuffer + sizeof(sce_header_t));
			_es_self_header(res->self.selfh);

			//Application info.
			res->self.ai = reinterpret_cast<app_info_t *>(res->scebuffer + res->self.selfh->app_info_offset);
			_es_app_info(res->self.ai);

			//Section infos.
			res->self.si = reinterpret_cast<section_info_t *>(res->scebuffer + res->self.selfh->section_info_offset);

			//SCE version.
			if(res->self.selfh->sce_version_offset != NULL)
			{
				res->self.sv = reinterpret_cast<sce_version_t *>(res->scebuffer + res->self.selfh->sce_version_offset);
			...
			auto len = static_cast<u32>(res->self.selfh->control_info_size);
			if(len > 0)
			{
				auto ptr = res->scebuffer + res->self.selfh->control_info_offset;
				res->self.cis = list_create();

				while(len > 0)
				{
					auto tci = reinterpret_cast<control_info_t *>(ptr);
	...
	res->metai = reinterpret_cast<metadata_info_t *>(scebuffer + sizeof(sce_header_t) + res->sceh->metadata_offset);
	res->metah = reinterpret_cast<metadata_header_t *>((u8 *)res->metai + sizeof(metadata_info_t));
	res->metash = reinterpret_cast<metadata_section_header_t *>((u8 *)res->metah + sizeof(metadata_header_t));
	...
	LIST_FOREACH(iter, ctxt->secs)
	{
		...
		if(sec->may_compr == TRUE)
		{
			if(sec->size > 0)
			{
				...
				{
					free(buf);
					_LOG_VERBOSE("Skipped compression of section %03d (0x%08lu >= 0x%08X)\n", i, size_comp, sec->size);
				}
			}
			else
...
static void _sce_calculate_hashes(sce_buffer_ctxt_t *ctxt)
{
	...
	LIST_FOREACH(iter, ctxt->secs)
	{
		sce_section_ctxt_t *sec = static_cast<sce_section_ctxt_t *>(iter->value);
...
BOOL sce_decrypt_header(sce_buffer_ctxt_t *ctxt, u8 *metadata_info, u8 *keyset)
{
	...
	if (metadata_info == nullptr)
	{
		...
		aes_crypt_cbc(&aes_ctxt, AES_DECRYPT, sizeof(metadata_info_t), iv, reinterpret_cast<u8 *>(ctxt->metai), reinterpret_cast<u8 *>(ctxt->metai));

		//Dump to file.
		_write_buffer("metai_dec.bin", reinterpret_cast<u8 *>(ctxt->metai), sizeof(metadata_info_t));
	}
	else
	{
		//Copy provided metadata info over SELF metadata.
		memcpy(reinterpret_cast<u8 *>(ctxt->metai), metadata_info, sizeof(metadata_info));
	}
	...
	aes_crypt_ctr(&aes_ctxt,
		ctxt->sceh->header_len - (sizeof(sce_header_t) + ctxt->sceh->metadata_offset + sizeof(metadata_info_t)),
		&nc_off, ctxt->metai->iv, sblk, reinterpret_cast<u8 *>(ctxt->metah), reinterpret_cast<u8 *>(ctxt->metah));
	...
	//Dump to file.
	_write_buffer("metah_dec.bin", reinterpret_cast<u8 *>(ctxt->metah), ctxt->sceh->header_len - (sizeof(sce_header_t) + ctxt->sceh->metadata_offset + sizeof(metadata_info_t)));
	...
	ctxt->keys = reinterpret_cast<u8 *>(ctxt->metash) + sizeof(metadata_section_header_t) * ctxt->metah->section_count;
	ctxt->keys_len = ctxt->metah->key_count * 0x10;

	//Dump to file.
	_write_buffer("keys_file.bin", reinterpret_cast<u8 *>(ctxt->metash) + sizeof(metadata_section_header_t) * ctxt->metah->section_count, ctxt->metah->key_count * 0x10);
	_write_buffer("signature.bin", static_cast<u8 *>(ctxt->keys) + ctxt->metah->key_count * 0x10, 0x30);
	...
	if (ctxt->sceh->header_type == SCE_HEADER_TYPE_SELF)
	{
		...
		opt_header_t *oh = reinterpret_cast<opt_header_t *>(ctxt->keys + ctxt->metah->key_count * 0x10);
		_es_opt_header(oh);
		list_add_back(ctxt->self.ohs, oh);
		while (oh->next != 0)
		{
			oh = reinterpret_cast<opt_header_t *>(reinterpret_cast<u8 *>(oh) + oh->size);
			_es_opt_header(oh);
			list_add_back(ctxt->self.ohs, oh);
		}

		//Signature.
		ctxt->sig = reinterpret_cast<signature_t *>(reinterpret_cast<u8 *>(oh) + oh->size);
	}
	else
		ctxt->sig = reinterpret_cast<signature_t *>(ctxt->keys + ctxt->metah->key_count * 0x10);
...
{
	u32 v = version >> 32;
	sprintf_s(_sce_tmp_vstr, "%02X.%02X", (v & 0xFFFF0000) >> 16, v & 0x0000FFFF);
...
{
	u16 h, l;
	sscanf_s(version, "%02X.%02X", &h, &l);
	return static_cast<u64>(h << 16 | l) << 32;
}
...
u64 sce_hexver_to_decver(u64 version)
{
	...
	sprintf_s(tmp, "%02X%02X", (v & 0xFFFF0000) >> 16, v & 0x0000FFFF);
	sscanf_s(tmp, "%d", &v);
	res = v*100;
...
control_info_t *sce_get_ctrl_info(sce_buffer_ctxt_t *ctxt, u32 type)
{
	LIST_FOREACH(iter, ctxt->self.cis)
	{
		auto ci = static_cast<control_info_t *>(iter->value);
		if(ci->type == type)
			return ci;
	}
...
opt_header_t *sce_get_opt_header(sce_buffer_ctxt_t *ctxt, u32 type)
{
	LIST_FOREACH(iter, ctxt->self.ohs)
	{
		auto oh = static_cast<opt_header_t *>(iter->value);
		if(oh->type == type)
			return oh;
	}
